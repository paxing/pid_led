...
class PID_LED():
    ...
    def apply(self, duration=120, delta_t=1, avg_size = 5, bin_size=5, zn_tuning = False, live = True):
        """
        ...
        def calculate_background():
            """
            this function measure the light background for 5 s
            """
            self.led.value = 0
            background = 0
            for i in range(10):
        ...
        def execute_PID(raw, i):
            ...
            photoresistance -= self.background

            #photoresistance = self.sensor.value - self.background

            raw.append(photoresistance)
            present_value = sliding(raw, avg_size)
            self.photoresistance.append(present_value)
            present_error = self.Ref - present_value
        ...
        if live:
            raw = (avg_size-1)*[self.background] #  padding
    ...
    def plot(self, data_set = 'photoresistance', **kwargs):
        ...
        if data_set == 'photoresistance':
            plt.plot(self.time, self.photoresistance, **kwargs)
        elif data_set == 'out':
            plt.plot(self.time, self.out, **kwargs)



    def analysis(self, eps=0.04, min_time = 10):
        """
